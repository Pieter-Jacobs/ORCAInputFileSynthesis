...
class DFTCalculation(Calculation):
    """Initializes a single point Density Functional Theory (DFT) based calculation.
    ...
    def __init__(self, xyz, molecule_file, molecule_type, add_solvation=False,
        ...
        self.grid_type = self.choose_grid_type() # Always choose a defgrid type 

    def process_keywords(self):
        """Implements all necessary keywords for a DFT calculation"""
    ...
    def process_input_blocks(self):
        """Implements all necessary input blocks for a DFT calculation"""

        # Using meta-gg of libxc functionals needs autotrah off, as it already uses dft-nl correction by default
        if 'b97m' in self.functional:
            self.input_blocks.append("%scf AutoTRAH false end")
            
        #  We parameterize NL if used
        if self.nl_correction is not None:
            self.input_blocks.append("%method NLb 5.0 end")
            self.input_blocks.append("%scf AutoTRAH false end")

        # We parameterize dispersion if used
        ...
        # If the density functional uses mp2 theory, we define its density
    ...
    def choose_functional(self, functional_type = None, molecule_type = None):
        """Randomly chooses a density functional and keeps account of the type of functional."""
        functional = None
        ...
        # DLPNO is only implemented for rhf, so we keep the calling function until we get a non-DLPNO functional 
    ...
    def choose_ri_approximation(self):
        """Chooses a random, viable RI approximation"""
        ri_approximation = None
        # Non-hybrids can only use the most basic ri approximation
    ...
    def choose_grid_type(self):
        """Chooses a random grid type"""
        grid_type = ORCADocumentationHandler.choose_random_keyword(keywords_simple_input.dft_grid_keywords) if not 'm06' in self.functional else 'defgrid3' # Frequency calcs need defgrid3
    ...
    def get_random_non_hybrid_functional(self):
        """Randomly chooses a non-hybrid functional"""
    ...
    def get_random_hybrid_functional(self):
        """Randomly chooses a hybrid functional"""
    ...
    def get_random_double_hybrid_functional(self):
        """Randomly chooses a double-hybrid functional"""
    ...
    def determine_hf_type(self):
        """Chooses the appropriate hartree fock based on the basis set and functional."""
        hf_type = super().determine_hf_type()

        # Gradient calculations are not implemented for ROHF
    ...
    def choose_non_local_correction(self):
        """Randomly chooses an NL correction"""        
    ...
    def choose_dispersion_correction(self):
        """Randomly chooses a dispersion method""" 
    ...
    def functional_is_range_seperated(self):
        """Returns whether a functional is range-seperated chooses""" 
    ...
    def functional_uses_mp2(self):
        """Returns whether a functional uses MP2 theory""" 
    ...
    def functional_uses_dlpno(self, functional=None):
        """Returns whether a functional uses DLPNO""" 
    ...
    def add_aux_basis_sets_to_keywords(self):
        ...
        # Add the appropriate basis sets for RI approximations
        ...
        # If our functional uses RI or DLPNO, we need a /c basis set
